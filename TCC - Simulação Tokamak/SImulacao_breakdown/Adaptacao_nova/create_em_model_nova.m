function [Maa,Mvv,Mav,Mva,Raa,Rvv] = create_em_model_nova

%% Loading TCABR coils
psiem = load('/home/kevi/Área de Trabalho/TCC - Simulação Tokamak/SImulação_breakdown/green_table/green_table_65x65.mat');
psiemh = load('/home/kevi/Área de Trabalho/TCC - Simulação Tokamak/SImulação_breakdown/green_table/green_table_129x129.mat');
Coil = load('/home/kevi/Área de Trabalho/TCC - Simulação Tokamak/SImulação_breakdown/Adaptacao_nova/nova_coils');
C = Coil([1 1:4],:)
cname = {'OH' 'E1' 'E2' 'E3' 'E4' 'E5' 'E6' 'E7' 'E8' 'E9' 'E10' 'F1' 'F2' 'F3' 'F4' 'D1' 'D2'};

%% Active-active coil coupling
% Resistance Matrix
Raa = 1.68e-8*Coil(:,4)*2*pi.*Coil(:,1)./(Coil(:,3).*Coil(:,4)); %Surface area divided by the number of turns
Raa = diag([sum(Raa(1:4)); Raa(1:4)]);

%Inductance matrix
L = self_inductance_nova(C(:,1),C(:,2).*C(:,3));
L(1,1) = 0;
for ioh = 1:7
    L(1,1) = L(1,1) + max(interp2(psiemh.r,psiemh.z,Coil(ioh,4)*psiemh.OH.G.',linspace(Coil(ioh,1)*0.8,Coil(ioh,1)*1.2,1001),Coil(ioh,2)*ones(1,1001),'cubic'));
end
Maa = diag(L);
