...
pion = pe/1.2;     % a  pressão de ions (pion) começa igua a pressão de eletros
Jphi = ones(size(rg,1),size(rg,2),Ntime+1); %definindo a densidade de corrente na direção toroidal como um valor inical constante bem pequeno 
...
v_en = 7.89e11*p0;
v_ei = out.v_ei;
Jphi = 10e-6*Jphi.*(v_ei+v_en).*Ephi; 
...
%     end
    ...
    interuptor1 = 0;
    Jphi(:,:,it+1) = Jphi(:,:,it) + dt*(n(:,:,it)*e^2/me.*Ephi +Jphi(:,:,it).*(s)  );
    % Plasma density
    laplaciano = 0*D*Polar_laplaciano(n(:,:,it),dR,dtheta,raio);
    ...
    dn = pe(:,:,it) + dt*(0.1*auxx - 2*n(:,:,it)*e^2/mi.*rho.*(pe(:,:,it) - pion(:,:,it)));
    ...
    pion(:,end,it+1) = e*Te0*n0; %pe = K*m^{-3}*C j = m^{-3}*C*m*s^{-1}
    % Transport coefficients
    iuu=it;
    out = resistivity_nova0(n(:,:,iuu),pe(:,:,iuu)./n(:,:,iuu)/e,1);
    v_ei = out.v_ei;
    v_en = 7.89e11*(ng-n(:,:,iuu))*(1.38e-23)*300;   
    rho  = out.eta_par;
    v_ion = ligav_ion*(a.*abs(Jphi(:,:,iuu))/e./n(:,:,iuu));
    v_loss = ligav_ion*(abs(Jphi(:,:,iuu).*Bphi)./sqrt(Br.^2 + Bphi.^2 + Bz.^2)./n(:,:,iuu)/e./Leff);
